...
function startQuiz() {
  ...
  let textheading = document.getElementById("text-heading");
  ...
  countForQuestion.innerText = `Question ${count + 1} `;
...
function loadQuestion() {
  const questionObject = quizData[currentQuestionIndex];
...
for (let i = 0; i < 4; i++) {
  ...
  optionButton.addEventListener("click", function (event) {
    ...
    if (selectedOption.innerText == quizData[currentQuestionIndex].answer) {
      score++;
      ...
      for (let i = 0; i < correctOption.length; i++) {
        if (
          correctOption[i].innerText === quizData[currentQuestionIndex].answer
        ) {
...
nextbtn.addEventListener("click", function () {
  ...
  for (let i = 0; i < correctOption.length; i++) {
    if (correctOption[i].innerText === quizData[currentQuestionIndex].answer) {
